...
bool characterInit(const char* name) {
  ...
  if (!mf) { Serial.printf("[char] manifest not found: %s\n", mpath); return false; }

  JsonDocument doc;
  DeserializationError err = deserializeJson(doc, mf);
  mf.close();
  if (err) { Serial.printf("[char] manifest parse: %s\n", err.c_str()); return false; }
...
void characterInvalidate() {
  if (!loaded) return;
  if (textMode) { uint8_t s = curState; curState = 0xFF; characterSetState(s); return; }
  if (gifOpen)  { gif.close(); gifOpen = false; }
  animPauseUntil = 0;
...
void characterSetState(uint8_t s) {
  ...
  if (textMode) { curState = s; textFrame = 0; textNext = 0; return; }
