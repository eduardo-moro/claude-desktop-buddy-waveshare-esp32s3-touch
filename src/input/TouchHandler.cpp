...
void TouchHandler::setUiOrientation(BoardConfig::UiOrientation orientation) {
  if (uiOrientation_ == orientation) return;
...
bool TouchHandler::poll(TouchEvent &event) {
  ...
  const uint16_t rawLong  = static_cast<uint16_t>(((data[2] & 0x0F) << 8) | data[3]);
  const uint16_t rawShort = static_cast<uint16_t>(((data[4] & 0x0F) << 8) | data[5]);
  const uint16_t physX    = clampPhysicalX(rawShort);
  const uint16_t physY    = clampPhysicalY(
      rawLong >= BoardConfig::PANEL_NATIVE_HEIGHT
          ? 0
          : static_cast<uint16_t>(BoardConfig::PANEL_NATIVE_HEIGHT - 1 - rawLong));
  ...
  switch (uiOrientation_) {
    ...
    case BoardConfig::UiOrientation::PortraitFlipped:
      event.x = static_cast<uint16_t>(BoardConfig::PANEL_NATIVE_WIDTH  - 1 - physX);
      event.y = static_cast<uint16_t>(BoardConfig::PANEL_NATIVE_HEIGHT - 1 - physY);
      break;
    case BoardConfig::UiOrientation::Landscape:
      event.x = clampDisplayX(static_cast<uint16_t>(BoardConfig::PANEL_NATIVE_HEIGHT - 1 - physY));
      event.y = clampDisplayY(physX);
      break;
    case BoardConfig::UiOrientation::LandscapeFlipped:
    default:
      event.x = clampDisplayX(physY);
      event.y = clampDisplayY(static_cast<uint16_t>(BoardConfig::PANEL_NATIVE_WIDTH - 1 - physX));
      break;
  }
