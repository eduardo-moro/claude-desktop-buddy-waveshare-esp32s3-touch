...
#define UI_FONT_SM()  do { spr.setFont(FONT_SM);  spr.setFontFallback(FONT_SYM); spr.setTextSize(1); } while(0)
#define UI_FONT_MD()  do { spr.setFont(FONT_MD);  spr.setFontFallback(FONT_SYM); spr.setTextSize(1); } while(0)
#define UI_FONT_OFF() do { spr.clearFont(); } while(0)
...
// Gruvbox dark palette — used for the UI when in ASCII buddy mode
static const Palette GRUVBOX_PAL = { 0xFDE5, 0x2945, 0xEED6, 0xACD0, 0xFC03 };
...
// UI always uses gruvbox — the GIF character only owns the buddy zone
static const Palette& uiPal() { return GRUVBOX_PAL; }
...
// ── Stats / settings (header-only, included once here) ───────────────────────
#include "stats.h"
#include "xfer.h"
...
// Colors
static constexpr uint16_t HOT  = 0xFA46;  // gruvbox red-dim  #fb4934
static constexpr uint16_t PANEL = 0x39C6; // gruvbox bg1      #3c3836
...
static void drawPasskey() {
  ...
  spr.print("BLUETOOTH PAIRING - enter on desktop:");
  UI_FONT_OFF();
  spr.setTextSize(3);
...
static void drawZoneHints(const Palette& p, const char* aLbl = "A", const char* bLbl = "B") {
  ...
  int bx = ax - 68 - bW/2;
  spr.setCursor(ax, 2); spr.print(aLbl);
  spr.setCursor(bx, 2); spr.print(bLbl);
  spr.drawFastHLine(TXZ + 4, 14, TW - 8, p.textDim);
}

// ── Menus ─────────────────────────────────────────────────────────────────────
static const char* menuItems[] = { "settings","turn off","help","about","demo","close" };
static const uint8_t MENU_N = 6;

static const char* settingsItems[] = {
  "brightness","sound","bluetooth","wifi","led","transcript","ascii pet","sleep","reset","back"
};
static const uint8_t SETTINGS_N = 10;
  ...
  switch (idx) {
    ...
    case 7: {
      ...
      for (uint8_t i = 0; i < N; i++) {
        if (sleepOpts[i] == cur) { next = sleepOpts[(i + 1) % N]; break; }
      }
      s.sleepMins = next;
    } break;
    case 8: resetOpen = true; resetSel = 0; resetConfirmIdx = 0xFF; return;
    case 9: settingsOpen = false; return;
  }
  settingsSave();
}
...
static void applyReset(uint8_t idx) {
  ...
  if (!armed) { resetConfirmIdx = idx; resetConfirmUntil = now + 3000; return; }
  if (idx == 0) {
...
void menuConfirm() {
  switch (menuSel) {
    ...
    case 1: BoardConfig::releaseBatteryPowerHold(); delay(200); ESP.restart(); break;
    case 2:
    case 3:
      menuOpen = false;
      displayMode = DISP_INFO;
      infoPage = (menuSel == 2) ? 1 : 4;
      break;
...
static void drawMenu() {
  ...
  for (int i = 0; i < MENU_N; i++) {
    ...
    if (i == 4) { spr.setTextColor(sel ? p.text : p.textDim, p.bg); spr.print(dataDemo() ? "  on" : "  off"); }
  }
  drawZoneHints(p);
}
...
static void drawSettings() {
  ...
  for (int i = 0; i < SETTINGS_N; i++) {
    ...
    if (i == 0) {
      ...
      spr.setTextColor(vals[i-1] ? GREEN : valColor, p.bg);
      ...
      if (m == 0) spr.print("off");
      else        spr.printf("%um", m);
    }
  }
...
static void drawReset() {
  ...
  spr.print("RESET");
  spr.drawFastHLine(TXZ + 4, 14, TW - 8, HOT);
  UI_FONT_SM();
  for (int i = 0; i < RESET_N; i++) {
    ...
    if (armed) spr.setTextColor(HOT, p.bg);
...
static void termPush(const char* s, bool fresh = true) {
  ...
  for (int i = 0; i < TERM_LINES; i++) termFresh[i] = false;
...
static void termUpdate() {
  if (tama.lineGen == lastLineGen) return;
...
static void drawInfo() {
  ...
  auto ln = [&](int& y, uint16_t c, const char* fmt, ...) {
    ...
    spr.setCursor(TXZ+4, y); spr.print(b); y += 10;
  };
  // Medium section header helper
  auto hd = [&](int& y, const char* label) {
    UI_FONT_MD();
    spr.setTextColor(p.body, p.bg);
    spr.setCursor(TXZ+4, y); spr.print(label); y += 16;
  };

  // Page header
  UI_FONT_MD();
  spr.setTextColor(p.text, p.bg);
  spr.setCursor(TXZ+4, 2); spr.print("Info");
  UI_FONT_SM();
  spr.setTextColor(p.textDim, p.bg);
  spr.setCursor(TXZ+TW-30, 2); spr.printf("%u/%u", infoPage+1, INFO_PAGES);
  int y = 18;
  ...
  if (infoPage == 0) {
    ...
    ln(y, p.textDim,"  hold 3s: power off"); y+=4;
    ...
    ln(y, p.textDim,"ble       %s", !bleConnected() ? "-" : bleSecure() ? "encrypted":"OPEN");
    ...
    hd(y, "DEVICE");
    if (hasBat) {
      UI_FONT_MD(); spr.setTextColor(p.text, p.bg);
      spr.setCursor(TXZ+4, y); spr.printf("%u%%", bat.percent);
      UI_FONT_SM(); spr.setTextColor(p.textDim, p.bg);
      spr.setCursor(TXZ+50, y+3); spr.printf("%.2fV", bat.voltage);
      y += 18;
    }
    ln(y, p.textDim,"heap   %uKB", ESP.getFreeHeap()/1024);
    uint32_t up = millis()/1000;
    ln(y, p.textDim,"uptime %luh%02lum", up/3600, (up/60)%60);
    if (ownerName()[0]) ln(y, p.textDim,"owner  %s", ownerName());
    { static const uint8_t bLv[] = {20,40,60,80,100}; ln(y, p.textDim,"bright %u%%", bLv[brightLevel]); }
  } else {
    hd(y, "CREDITS");
    y += 2;
...
static void drawPet() {
  ...
  if (ownerName()[0]) { spr.print(ownerName()); spr.print("'s "); }
  spr.print(petName());
  UI_FONT_SM();
  spr.setTextColor(p.textDim, p.bg);
  spr.setCursor(TXZ+TW-30, 2); spr.printf("%u/%u", petPage+1, PET_PAGES);

  if (petPage == 0) {
    int y = 16;
    auto row = [&](const char* label, const char* fmt, ...) {
      char b[32]; va_list a; va_start(a,fmt); vsnprintf(b,sizeof(b),fmt,a); va_end(a);
      spr.setTextColor(p.textDim, p.bg); spr.setCursor(TXZ+4, y); spr.print(label);
      spr.setTextColor(p.text, p.bg); spr.setCursor(TXZ+90, y); spr.print(b); y += 12;
    };
    ...
    if (tok >= 1000000)   row("tokens", "%.1fM", tok/1000000.0f);
    else if (tok >= 1000) row("tokens", "%.1fK", tok/1000.0f);
    else                  row("tokens", "%lu",   (unsigned long)tok);
    row("today tok","%lu",   (unsigned long)tama.tokensToday);
  } else {
    int y = 16;
    spr.setTextColor(p.textDim, p.bg);
    auto ln = [&](const char* s) { spr.setCursor(TXZ+4, y); spr.print(s); y += 10; };
...
static void drawBuddyStats() {
  ...
  // Lv badge — right edge
  char lvbuf[8]; snprintf(lvbuf, sizeof(lvbuf), "Lv %u", stats().level);
  int lvw = strlen(lvbuf) * 6 + 4;
  int lvX = BZ - 2 - lvw;
  spr.fillRoundRect(lvX, CY - LVH / 2, lvw, LVH, 2, p.body);
  spr.setTextColor(p.bg, p.body);
  spr.setCursor(lvX + 2, CY - LVH / 2 + 1); spr.print(lvbuf);
  ...
  for (int i = 4; i >= 0; i--) {
    ...
    cx = 2 + ER + i * ERST;
    if (i < (int)energy) spr.fillCircle(cx, CY, ER, eCols[i]);
    else                 spr.drawCircle(cx, CY, ER, p.textDim);  // all at CY = Lv badge center
  }
  eRightEdge = 2 + ER + 4 * ERST + ER;  // right edge of last energy circle

  // Mood hearts — centered in the gap between energy and Lv
  uint8_t mood = statsMoodTier();
  uint16_t moodCol = (mood >= 3) ? BUDDY_HEART : (mood >= 2) ? BUDDY_YEL : p.textDim;
  ...
  uint32_t tok = stats().tokens;
  char tokbuf[12];
  if (tok >= 1000000)   snprintf(tokbuf, sizeof(tokbuf), "%.1fM", tok / 1000000.0f);
  else if (tok >= 1000) snprintf(tokbuf, sizeof(tokbuf), "%.1fK", tok / 1000.0f);
...
static void drawStatusBar() {
  ...
  // BLE indicator — top-left
  spr.setTextColor(bleConnected() ? GREEN : p.textDim, p.bg);
  spr.setCursor(2, 2); spr.print(bleConnected() ? "BLE" : "---");
  ...
  if (millis() - lastBatMs > 30000) {
    ...
    if (BoardConfig::readBatteryStatus(b)) batPct = b.percent;
  }
  if (batPct > 0) {
    char batBuf[6]; snprintf(batBuf, sizeof(batBuf), "%u%%", batPct);
    int bw = spr.getStrWidth(batBuf);
    spr.setTextColor(batPct < 20 ? HOT : p.textDim, p.bg);
    spr.setCursor(BZ - 2 - bw, 2); spr.print(batBuf);
  }
  ...
  for (uint8_t i = 0; i < FED_N; i++) {
    ...
    if (i < fed) spr.fillCircle(cx, 5, FED_R, p.body);
    else         spr.drawCircle(cx, 5, FED_R, p.textDim);
  }
}
...
void triggerOneShot(PersonaState s, uint32_t durMs) {
  activeState = s; oneShotUntil = millis() + durMs;
}
...
void setup() {
  ...
  // Splash
  {
    ...
    if (ownerName()[0]) {
      ...
      spr.setTextSize(1);
      spr.setTextColor(p.textDim, p.bg); spr.drawString("a buddy appears", W/2, H/2 + 14);
    }
...
void loop() {
  ...
  if (btnA.isHeld() || btnB.isHeld()) {
    if (screenOff) {
      if (btnA.isHeld()) swallowA = true;
      if (btnB.isHeld()) swallowB = true;
    }
    wake();
  }
  ...
  if (btnA.wasReleasedEvent()) {
    brightnessFired = false; powerOffFired = false;
    if (!swallowA) {
      if (inPrompt) {
        char cmd[96];
  ...
  if (btnB.wasReleasedEvent()) {
    if (!swallowB) {
      if (inPrompt) {
        ...
        msgScroll = (msgScroll >= 20) ? 0 : msgScroll + 1;
      }
    }
    swallowB = false;
  }
  ...
  if (!btnB.isHeld()) menuLongFired = false;
